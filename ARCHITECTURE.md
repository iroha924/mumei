...
│   ├── subagent-cost-log.sh  # SubagentStop: agent_id-based subagent jsonl usage extraction + review-iteration stamp (real_count + 1)
...
| R2   | review       | PreToolUse(Bash)         | `git push` while latest review verdict is `MAJOR_ISSUES`, OR a clearing verdict (`PASS` / `NEEDS_IMPROVEMENT`) is not backed by a reviewer-execution trace in `cost-log.jsonl` — a hollow review (a required reviewer left no SubagentStop cost-log record stamped with the gating review's history-derived iteration, OR the resolved gating review is itself `MAJOR_ISSUES`); `mumei_review_trace_ok` in `hooks/_lib/review.sh` | `hooks/pre-bash-guard.sh`     |
...
| L-R2 | plan-vehicle | PreToolUse(Bash)         | `git push` while latest plan-vehicle review verdict is `MAJOR_ISSUES`, OR a clearing verdict lacks a reviewer-execution trace in `cost-log.jsonl` (hollow review) — deny                                                                                                                                                                                                                                                          | `hooks/pre-bash-guard.sh`     |
