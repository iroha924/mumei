...
- **No historical annotations.** Never write version-tracking notes such as "added in REQ-N", "filled in during Wave N", or "revised (2026-MM-DD)". Readers only want the current behavior; history lives in git log and in the PR that landed the change.
...
- **No option lists up front.** Write only the adopted procedure. Alternatives and rejected options go in the PR description.
