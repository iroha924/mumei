...
| R2   | review       | PreToolUse(Bash)         | `git push` while latest review verdict is `MAJOR_ISSUES`, OR a clearing verdict (`PASS` / `NEEDS_IMPROVEMENT`) is not backed by reviewer execution — a hollow review (a baseline reviewer left no SubagentStop cost-log record for this feature, OR the resolved gating review is itself `MAJOR_ISSUES`); presence-based, see `mumei_review_trace_ok` in `hooks/_lib/review.sh` | `hooks/pre-bash-guard.sh`     |
