...
| R2   | review       | PreToolUse(Bash)         | `git push` while latest review verdict is `MAJOR_ISSUES`, OR a clearing verdict (`PASS` / `NEEDS_IMPROVEMENT`) is not backed by a reviewer-execution trace in `cost-log.jsonl` — a hollow review (the always-on reviewer left no SubagentStop cost-log record in the gating review's window); `mumei_review_trace_ok` in `hooks/_lib/review.sh`                 | `hooks/pre-bash-guard.sh`     |
