...
mumei also writes `.claude/agent-memory-local/` (per-issue-validator decision cache, added to project-root `.gitignore` by `/mumei:init`).

### Implications for git

mumei itself does not push these files anywhere. `/mumei:init` writes `.mumei/.gitignore` that ignores **only** per-developer state (`current`, `specs/*/state.json`); everything else under `.mumei/` — `scratch/`, `specs/*/{requirements,design,tasks}.md`, `spec-reviews/`, `reviews/`, `cost-log.jsonl`, `archive/` — is intentionally **tracked** for team handoff. If you do not want these committed, edit `.mumei/.gitignore` after `/mumei:init` or add `.mumei/` to the project-root `.gitignore`.
