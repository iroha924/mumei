...
`/mumei:compose` で新規機能を起動すると、方式の選択時に、定量的な目安（`spec` は `> 3 files OR > 100 lines`、`plan` はその逆）を各選択肢の説明に含めて見当をつけやすくします。さらに gather のメモが添付されている場合は、mumei がメモの AC 数と Goal 節を読んで推奨する方式を計算し、別ステップで「推奨で進める / 変更する」を確認します。最終判断は常にユーザーの手にあり、推奨はあくまで参考です。
