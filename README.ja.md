...
`CLAUDE.md` のルール、システムプロンプト、「先にテストを実行して」——これらはすべて「お願い」であり、能力の高いエージェントはプレッシャーがかかると平気で迂回します。mumei は、守りたい基準をプロンプトから OS の境界へ移します。そこではフックが、プロジェクトを変更する操作——編集・コミット・プッシュ・フェーズ遷移——を検査し、不変条件を破るものを拒否します。mumei が「お願い」ではなく「強制」する 4 つ:

- **チャットではなく、ハーネス。** フェーズ・Wave・コミット・プッシュ、そしてレビュー工程の全体を、フックが決定論的に駆動します。エージェントがプロンプトで切り抜けることはできません。唯一の抜け道は、明示的に設定する `MUMEI_BYPASS=1` の 1 つだけです。これは**あなたが**立てる環境変数であり、**エージェントに立てさせない**ための唯一の仕掛けでもあります。Claude Code の設定ファイルに書き込もうとすれば拒否され、有効な状態で始まったセッションは開口一番それを申告します。
- **エージェントが黙って弱められないテスト。** コミット時、テストは汚れていないクリーンな `HEAD` のワークツリーで再実行されます。手元に仕込んだ細工は何も効きません。golden ファイルは読み込む前に `HEAD` の内容へ戻されます。プロパティテストは実装を見ずに——仕様と関数シグネチャだけから——書かれ、そのまま凍結されます。そして差分そのものを読み、「赤いチェックを緑に取り替える手口」を探します。
- **仕様駆動開発は、使いたければ。** 仕様を「生成する」ツールは数多くありますが、mumei はエージェントに「その仕様どおりに作らせ」ます——要件 → 設計 → タスク（それぞれ独立にレビュー）→ 一度きりの承認ゲート → Wave 単位の実装。フェーズの飛ばしや壊れた Wave のコミットは物理的にブロックされます。ただしこれは **3 つある入口の 1 つであって、通行料ではありません**。`/mumei:review` は同じレビューエンジンと同じ検出器を、いまの差分にそのまま当てます——**`.mumei` を作らず、状態を持たず、儀式もなし**。plan vehicle は Claude Code 標準の plan モードをそのまま包みます。**あなたのやり方はそのままに、強制だけを持ち込めます。**
