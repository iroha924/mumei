...
| E1   | implement    | PreToolUse(Edit)         | Editing a production file (both vehicles) while the artifact is missing, OR its `## Open Questions` section is absent, has an unchecked `- [ ]`, or is non-`None` prose                    | `hooks/pre-edit-guard.sh`     |
